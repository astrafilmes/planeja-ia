// FONTE CANÔNICA (front) de normalização de nomes de secretaria.
// Deve ser mantida byte-a-byte igual a vps-worker/src/m2a/norm-sec.js (worker).
...
export function normSec(txt: unknown): string {
  return String(txt ?? "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toUpperCase()

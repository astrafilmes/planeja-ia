...
-- Backfill: restaurar fornecedor_nome dos contratos a partir de qualquer m2a_atas que ainda tenha o nome.
WITH best_ata AS (
  SELECT DISTINCT ON (m2a_ata_id)
         m2a_ata_id,
         fornecedor_nome
    FROM public.m2a_atas
   WHERE NULLIF(trim(coalesce(fornecedor_nome,'')),'') IS NOT NULL
   ORDER BY m2a_ata_id, synced_at DESC NULLS LAST
)
UPDATE public.contratos co
   SET fornecedor_nome = b.fornecedor_nome
  FROM best_ata b
 WHERE co.m2a_ata_id = b.m2a_ata_id
   AND co.deleted_at IS NULL
   AND NULLIF(trim(coalesce(co.fornecedor_nome,'')),'') IS NULL;

-- Também limpar empty-string em m2a_atas para evitar mascarar dados bons no futuro.
UPDATE public.m2a_atas
   SET fornecedor_nome = NULL
 WHERE NULLIF(trim(coalesce(fornecedor_nome,'')),'') IS NULL

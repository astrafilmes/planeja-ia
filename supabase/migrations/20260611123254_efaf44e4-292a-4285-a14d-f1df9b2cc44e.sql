...
BEGIN
  SELECT m2a_processo_id INTO v_db_m2a_processo_id
   ...
   WHERE id = p_processo_id AND deleted_at IS NULL;
  ...
  WITH src AS (
    SELECT (a->>'id_ata') AS m2a_ata_id,
           ...
           NULLIF(trim(coalesce(a->'fornecedor'->>'nome','')), '') AS fornecedor_nome,
           NULLIF(trim(coalesce(a->'fornecedor'->>'cnpj','')), '') AS fornecedor_cnpj
  ...
  ups AS (
    ...
    ON CONFLICT (processo_id, m2a_ata_id) DO UPDATE
      SET numero_ata = EXCLUDED.numero_ata,
          fornecedor_nome = coalesce(EXCLUDED.fornecedor_nome, public.m2a_atas.fornecedor_nome),
          fornecedor_cnpj = coalesce(EXCLUDED.fornecedor_cnpj, public.m2a_atas.fornecedor_cnpj),
          synced_at = now()
    RETURNING 1
  )
    ...
    DELETE FROM public.m2a_atas
     WHERE processo_id = p_processo_id
       AND m2a_ata_id NOT IN (
         SELECT (a->>'id_ata') FROM jsonb_array_elements(v_atas) a WHERE (a->>'id_ata') IS NOT NULL
       )
    RETURNING 1
  )
  ...
  SELECT count(*) FILTER (WHERE inserted), count(*) FILTER (WHERE NOT inserted)
    INTO v_itens_inserted, v_itens_updated FROM ups;

  WITH del AS (
    DELETE FROM public.m2a_itens
     WHERE processo_id = p_processo_id
       AND m2a_item_id NOT IN (
         SELECT (i->>'id_item') FROM jsonb_array_elements(v_itens) i WHERE (i->>'id_item') IS NOT NULL
       )
    RETURNING 1
  )
  ...
  DELETE FROM public.m2a_contratos_snapshot
   WHERE processo_id = p_processo_id
     AND m2a_contrato_id NOT IN (
       SELECT (c->>'id_contrato_m2a') FROM jsonb_array_elements(v_contratos) c WHERE (c->>'id_contrato_m2a') IS NOT NULL
     );
     ...
     WHERE co.id = ci.contrato_id
       ...
       AND NOT EXISTS (
         SELECT 1 FROM public.m2a_itens mi
          WHERE mi.processo_id = p_processo_id AND mi.m2a_item_id = ci.m2a_item_id
       )
    RETURNING 1
  )
  SELECT count(*) INTO v_orfaos_limpos FROM cleared;

  -- Atualiza contratos locais com dados do payload do portal.
  -- Agora prioriza o fornecedor capturado DIRETAMENTE da linha do contrato
  -- (mais confiável que o da ata, que pode estar vazio).
  WITH ata_info AS (
    SELECT m2a_ata_id, numero_ata, NULLIF(trim(coalesce(fornecedor_nome,'')),'') AS fornecedor_nome
      FROM public.m2a_atas WHERE processo_id = p_processo_id
  ),
  src AS (
    SELECT (c->>'id_contrato_m2a') AS m2a_contrato_id,
           (c->>'numero_contrato') AS numero_contrato,
           (c->>'id_ata') AS id_ata,
           NULLIF(trim(coalesce(c->>'fornecedor_nome','')),'') AS fornecedor_contrato,
           NULLIF(trim(coalesce(c->>'objeto','')),'') AS objeto_contrato
      FROM jsonb_array_elements(v_contratos) c
  ),
  joined AS (
    SELECT co.id,
           s.id_ata,
           ai.numero_ata,
           coalesce(s.fornecedor_contrato, ai.fornecedor_nome) AS fornecedor_nome,
           s.objeto_contrato
      FROM src s
      ...
      JOIN public.contratos co
       ...
       AND co.deleted_at IS NULL
       AND (co.m2a_contrato_id = s.m2a_contrato_id
  ...
  upd AS (
    UPDATE public.contratos co
       SET m2a_ata_numero = coalesce(j.numero_ata, co.m2a_ata_numero),
           fornecedor_nome = coalesce(j.fornecedor_nome, NULLIF(trim(coalesce(co.fornecedor_nome,'')),''), co.fornecedor_nome),
           m2a_ata_id = coalesce(j.id_ata, co.m2a_ata_id),
           objeto = coalesce(NULLIF(trim(coalesce(co.objeto,'')),''), j.objeto_contrato, co.objeto)
      FROM joined j
     WHERE co.id = j.id
       AND ((j.numero_ata IS NOT NULL AND co.m2a_ata_numero IS DISTINCT FROM j.numero_ata)
         OR (j.fornecedor_nome IS NOT NULL AND NULLIF(trim(coalesce(co.fornecedor_nome,'')),'') IS DISTINCT FROM j.fornecedor_nome)
         OR (j.id_ata IS NOT NULL AND co.m2a_ata_id IS DISTINCT FROM j.id_ata)
         OR (j.objeto_contrato IS NOT NULL AND NULLIF(trim(coalesce(co.objeto,'')),'') IS NULL))
    RETURNING 1
  )
  ...
  UPDATE public.contrato_itens ci
    ...
    FROM public.m2a_itens mi, public.contratos co
   WHERE mi.processo_id = p_processo_id
  ...
  WITH legacy AS (
    SELECT ci.id AS ci_id, ci.quantidade, public.normalize_numero_item(ci.numero_item) AS norm_num, lower(trim(coalesce(co.fornecedor_nome,''))) AS forn
     ...
     WHERE co.processo_id = p_processo_id AND co.deleted_at IS NULL AND ci.m2a_item_id IS NULL
  ...
  candidates AS (
    SELECT mi.m2a_item_id, mi.numero_item, mi.descricao, mi.unidade, mi.valor_unitario, public.normalize_numero_item(mi.numero_item) AS norm_num, lower(trim(coalesce(ma.fornecedor_nome,''))) AS forn
      FROM public.m2a_itens mi JOIN public.m2a_atas ma ON ma.processo_id = mi.processo_id AND ma.m2a_ata_id = mi.m2a_ata_id
     WHERE mi.processo_id = p_processo_id
  ),
  matched AS (
    SELECT l.ci_id, l.quantidade, c.m2a_item_id, c.numero_item, c.descricao, c.unidade, c.valor_unitario, count(*) OVER (PARTITION BY l.ci_id) AS n
      FROM legacy l JOIN candidates c ON c.norm_num = l.norm_num AND c.forn = l.forn AND l.forn <> ''
  ),
  unique_match AS (SELECT DISTINCT ci_id, quantidade, m2a_item_id, numero_item, descricao, unidade, valor_unitario FROM matched WHERE n = 1),
  upd2 AS (
    UPDATE public.contrato_itens ci
      ...
      FROM unique_match um WHERE ci.id = um.ci_id
    RETURNING 1
  )
  ...
  WITH legacy AS (
    SELECT ci.id AS ci_id, ci.quantidade, public.normalize_numero_item(ci.numero_item) AS norm_num
      FROM public.contrato_itens ci JOIN public.contratos co ON co.id = ci.contrato_id
     WHERE co.processo_id = p_processo_id AND co.deleted_at IS NULL AND ci.m2a_item_id IS NULL
       AND public.normalize_numero_item(ci.numero_item) IS NOT NULL
  ),
  candidates AS (
    SELECT mi.m2a_item_id, mi.numero_item, mi.descricao, mi.unidade, mi.valor_unitario, public.normalize_numero_item(mi.numero_item) AS norm_num
      FROM public.m2a_itens mi WHERE mi.processo_id = p_processo_id
  ),
  matched AS (
    SELECT l.ci_id, l.quantidade, c.m2a_item_id, c.numero_item, c.descricao, c.unidade, c.valor_unitario, count(*) OVER (PARTITION BY l.ci_id) AS n_ci
      FROM legacy l JOIN candidates c ON c.norm_num = l.norm_num
  ),
  unique_match AS (SELECT DISTINCT ci_id, quantidade, m2a_item_id, numero_item, descricao, unidade, valor_unitario FROM matched WHERE n_ci = 1),
  upd3 AS (
    UPDATE public.contrato_itens ci
       SET m2a_item_id = um.m2a_item_id,
           numero_item = CASE WHEN coalesce(ci.numero_item,'') = '' THEN um.numero_item ELSE ci.numero_item END,
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    'contrato_item_id', ci.id, 'contrato_id', ci.contrato_id,
    'numero_contrato', co.numero_contrato, 'numero_item', ci.numero_item,
    'descricao', ci.descricao
  )), '[]'::jsonb) INTO v_ambiguous
    FROM public.contrato_itens ci JOIN public.contratos co ON co.id = ci.contrato_id
   WHERE co.processo_id = p_processo_id AND co.deleted_at IS NULL AND ci.m2a_item_id IS NULL;

  UPDATE public.processos SET m2a_sync_at = now(), updated_at = now() WHERE id = p_processo_id;

  RETURN jsonb_build_object(
    'atas_upserted', v_atas_upserted, 'atas_removed', v_atas_removed,
    'itens_inserted', v_itens_inserted, 'itens_updated', v_itens_updated, 'itens_removed', v_itens_removed,
    'contratos_snapshot', v_snapshot_upserted, 'snapshots_orfaos_removidos', v_snapshot_orfaos,
    'contratos_atualizados', v_contratos_atualizados, 'orfaos_limpos', v_orfaos_limpos,
    'itens_relinkados', v_relinkados, 'itens_ambiguos', v_ambiguous, 'warnings', v_warnings
  );
END;
$function$;

-- Backfill: preenche fornecedor_nome em contratos vazios usando o snapshot bruto
-- do portal (raw->>'fornecedor_nome'), que agora é capturado direto da linha do
-- contrato no portal M2A.
UPDATE public.contratos co
   SET fornecedor_nome = sub.fornecedor_nome
  FROM (
    SELECT s.numero_contrato, s.m2a_contrato_id,
           NULLIF(trim(coalesce(s.raw->>'fornecedor_nome','')),'') AS fornecedor_nome
      FROM public.m2a_contratos_snapshot s
     WHERE NULLIF(trim(coalesce(s.raw->>'fornecedor_nome','')),'') IS NOT NULL
  ) sub
 WHERE NULLIF(trim(coalesce(co.fornecedor_nome,'')),'') IS NULL
   AND co.deleted_at IS NULL
   AND (co.m2a_contrato_id = sub.m2a_contrato_id
        OR lower(trim(coalesce(co.numero_contrato,''))) = lower(trim(coalesce(sub.numero_contrato,''))));
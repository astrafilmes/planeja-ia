  ...
  upd AS (
    UPDATE public.contratos c
     ...
     WHERE c.processo_id = p_processo_id
       ...
       AND (
         c.m2a_contrato_id = src.m2a_contrato_id
         OR lower(trim(coalesce(c.numero_contrato, ''))) = lower(trim(coalesce(src.numero_contrato, '')))
       )
       AND (
...
GRANT EXECUTE ON FUNCTION public.sync_m2a_contract_dates_from_snapshot(uuid) TO service_role;
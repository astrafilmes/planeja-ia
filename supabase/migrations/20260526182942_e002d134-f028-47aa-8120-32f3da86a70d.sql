...
-- Tabela 1: cabeçalho do upload (job de importação de contratos)
CREATE TABLE public.contrato_import_jobs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  original_filename text NOT NULL,
  upload_file_id uuid,
  status text NOT NULL DEFAULT 'preview', -- preview | autorizado | erro | cancelado
  empresa text,
...
-- Tabela 2: itens da planilha (uma linha = um item, editável)
...
-- Tabela 3: dotações em formato long (item x secretaria x dotação x quantidade)
...
-- Vínculo opcional do contrato gerado com o job de importação

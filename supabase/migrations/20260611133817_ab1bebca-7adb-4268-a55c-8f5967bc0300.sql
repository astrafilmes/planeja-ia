...
ALTER TABLE public.m2a_contratos_snapshot
  RENAME COLUMN sigla_secretaria TO secretaria_sigla;

-- Recriar as duas funções afetadas com a coluna nova
CREATE OR REPLACE FUNCTION public.next_contrato_numbers_batch_for_base(p_numero_base text, p_sec_sigla text, p_sec_num integer, p_qtd integer)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WITH candidatos AS (
    ...
    SELECT coalesce(snapshot_rows.sequencia, (snapshot_rows.m[1])::int) AS sequencia FROM (
      ...
      WHERE upper(coalesce(secretaria_sigla, v_sigla_norm)) = v_sigla_norm
...
-- sync_m2a_snapshot: atualizar somente o bloco que insere snapshot
...
BEGIN
  ...
  WITH src AS (
    SELECT DISTINCT ON (c->>'id_contrato_m2a')
           ...
           (c->>'sigla_secretaria') AS secretaria_sigla,
  ...
  ups AS (
    INSERT INTO public.m2a_contratos_snapshot
      (processo_id, m2a_contrato_id, numero_contrato, m2a_ata_id, secretaria_sigla, ano, sequencia, raw)
    SELECT p_processo_id, m2a_contrato_id, numero_contrato, m2a_ata_id, secretaria_sigla, ano, sequencia, raw
      FROM src
